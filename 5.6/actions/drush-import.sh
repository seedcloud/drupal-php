...
if [[ -d "${tmp_dir_files}/private" ]]; then
    rsync -rlt --delete --force "${tmp_dir_files}/private/" "${SEEDCLOUD_DIR_FILES}/private/"
fi

if [[ -d "${tmp_dir_files}/public" ]]; then
    rsync -rlt --delete --force "${tmp_dir_files}/public/" "${SEEDCLOUD_DIR_FILES}/public/"
elif [[ -d "${tmp_dir_files}" ]]; then
    rsync -rlt --delete --force "${tmp_dir_files}/" "${SEEDCLOUD_DIR_FILES}/public/"
fi
