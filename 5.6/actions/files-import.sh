...
if [[ -d "${tmp_dir}/private" ]]; then
    rsync -rlt --force "${tmp_dir}/private/" "${SEEDCLOUD_DIR_FILES}/private/"
fi

if [[ -d "${tmp_dir}/public" ]]; then
    rsync -rlt --force "${tmp_dir}/public/" "${SEEDCLOUD_DIR_FILES}/public/"
else
    rsync -rlt --force "${tmp_dir}/" "${SEEDCLOUD_DIR_FILES}/public/"
fi
