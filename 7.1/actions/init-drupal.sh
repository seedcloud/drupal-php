...
# Include seeedcloud.settings.php
if [[ ! -f "${settings_php}" ]]; then
    echo -e "<?php\n\n" > "${settings_php}"
fi

if [[ $( grep -ic "seeedcloud.settings.php" "${settings_php}" ) -eq 0 ]]; then
    chmod 644 "${settings_php}"
    echo -e "${disclaimer}" >> "${settings_php}"
    echo -e "include '${SEEDCLOUD_DIR_CONF}/seeedcloud.settings.php';" >> "${settings_php}"
fi

# Include seeedcloud.sites.php for Drupal 7 and 8.
if [[ "${DRUPAL_SITE}" != "default" ]]; then
    if [[ "${DRUPAL_VERSION}" == "8" ]] || [[ "${DRUPAL_VERSION}" == "7" ]]; then
        if [[ $( grep -ic "seeedcloud.sites.php" "${sites_php}" ) -eq 0 ]]; then
            echo -e "${disclaimer}" >> "${sites_php}"
            echo -e "include '${SEEDCLOUD_DIR_CONF}/seeedcloud.settings.php';" >> "${sites_php}"
        fi
    fi
fi
...
if [[ "${DRUPAL_VERSION}" == "8" ]]; then
    mkdir -p "${SEEDCLOUD_DIR_FILES}/config/sync_${DRUPAL_FILES_SYNC_SALT}"
fi
